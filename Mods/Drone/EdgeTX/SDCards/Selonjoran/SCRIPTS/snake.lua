...
local function run(event)
    ...
    local dir = direction
    -- JOELwindows7: Edit this for MT12 lol! was rud & ele
    if getValue('st') > 100 and direction ~= "left" then
        dir = "right"
        Head.dx = 1
        Head.dy = 0
    end
    if getValue('st') < -100 and direction ~= "right" then
        dir = "left"
        Head.dx = -1
        Head.dy = 0
    end
    if getValue('th') > 100 and direction ~= "down" then
        dir = "up"
        Head.dx = 0
        Head.dy = -1
    end
    if getValue('th') < -100 and direction ~= "up" then
        dir = "down"
        Head.dx = 0
        Head.dy = 1
    end
